...
function StorageService(options) {
  ...
  const awsOptionNames = ['StorageClass', 'CacheControl', 'ServerSideEncryption', 'SSEKMSKeyId', 'SSECustomerAlgorithm', 'SSECustomerKey', 'SSECustomerKeyMD5'];
...
StorageService.prototype.upload = function(container, req, res, options, cb) {
  ...
  if (this.ServerSideEncryption && !options.ServerSideEncryption) {
    options.ServerSideEncryption = this.ServerSideEncryption;
  }
  if (this.StorageClass && !options.StorageClass) {
    options.StorageClass = this.StorageClass;
  }
  if (this.CacheControl && !options.CacheControl) {
    options.CacheControl = this.CacheControl;
  }
  if (this.ServerSideEncryption && !options.ServerSideEncryption) {
    options.ServerSideEncryption = this.ServerSideEncryption;
  }
  if (this.SSEKMSKeyId && !options.SSEKMSKeyId) {
    options.SSEKMSKeyId = this.SSEKMSKeyId;
  }
  if (this.SSECustomerAlgorithm && !options.SSECustomerAlgorithm) {
    options.SSECustomerAlgorithm = this.SSECustomerAlgorithm;
  }
  if (this.SSECustomerKey && !options.SSECustomerKey) {
    options.SSECustomerKey = this.SSECustomerKey;
  }
  if (this.SSECustomerKey && !options.SSECustomerKeyMD5) {
    options.SSECustomerKeyMD5 = this.SSECustomerKeyMD5;
  }
